...
image::{code}/channels/diagrams/class/pub-sub.jpg[alt=PubSub Channel Class Diagram]
...
image::{code}/channels/diagrams/class/rendezvous.jpg[alt=Rendezvous Channel Class Diagram]
...
image::{code}/channels/class/executor-channel.jpg[alt=Executor Channel Class Diagram]
...
Spring Integration provides the ability to intercept the messages at specific stages around the `MessageChannel`'s send and receive operations. The `ChannelInteceptor` interface allows the message to be intercepted - much like AOP - before and after it is sent and/or received WITHOUT error, and finally as a completion phase whether or not an exception has occured.
