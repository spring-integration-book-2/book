...
A point-to-point channel (see Figure 12) ensures that there is only one consumer that receives the same message from the sender at any given time. Spring Integration provides several types of point-to-point channel implementations:
`QueueChannel`, `PriorityChannel`, `RendezvousChannel`, `DirectChannel`, `ExecutorChannel`, and `NullChannel`.

el.caption = Point-to-Point Channel
image::{code}/channels/diagrams/eip/point-to-point-channel.jpg[alt=The Point to Point Channel]
...
.Publish-subscribe Channel
image::{code}/channels/diagrams/eip/pubsub-channel.jpg[alt=The Publish Subscribe]
...
A message channel that only contains single type of objects or messages is called a data-typed channel.

.Data-Typed Channel
image::{code}/channels/diagrams/eip/data-typed-channel.jpg[alt=The Data Typed Channel]
...
----
include::{code}/channels/src/main/tdg2si/channels/DataTyped.java[]
----
...
.Invalid Message Channel
image::{code}/channels/invalid-message-channel.jpg[alt=The Invalid Message Channel]
...
.Dead Letter Channel
image::{code}/channels/diagrams/eip/dead-letter-channel.jpg[alt=The Dead Letter Channel]
...
.Channel Adapter
image::{code}/channels/diagrams/eip/channel-adapter.jpg[alt=The Channel Adapter]
...
.Messaging Bridge
image::{code}/channels/diagrams/eip/messaging-bridge.jpg[alt=The Messaging Bridge]
...
.Guaranteed Delivery
image::{code}/channels/diagrams/eip/guaranteed-delivery.jpg[alt=Guaranteed Delivery Pattern]
...
image::{code}/channels/diagrams/class/pub-sub-channel.jpg[alt=PubSub Channel Class Diagram]
...
image::{code}/channels/diagrams/class/queue-channel.jpg[alt=Queue Channel Class Diagram]
...
image::{code}/channels/diagrams/class/priority-channel.png[alt=Priority Channel Class Diagram]
...
image::{code}/channels/diagrams/class/rendezvous-channel.jpg[alt=Rendezvous Channel Class Diagram]
...
image::{code}/channels/diagrams/eip/rendezvous-request-reply.jpg[alt=Rendezvous Request Reply Diagram]
...
image::{code}/channels/diagrams/class/direct-channel.jpg[alt=Direct Channel Class Diagram]
...
image::{code}/channels/diagrams/class/executor-channel.jpg[alt=Executor Channel Class Diagram]
...
image::{code}/channels/diagrams/eip/load-balancer.jpg[alt=Load Balancing Handlers Diagram]
...
[source,java,indent=0]
----
include::{code}/channels/src/sample/LoadBalancingStrategy.java[]
----
...
image::{code}/channels/diagrams/class/null-channel.jpg[alt=Null Channel Class Diagram]
...
image::{code}/channels/diagrams/class/flux-message-channel.jpg[alt=Flux Message Channel Class Diagram]

== Channel Interceptors

Spring Integration provides the ability to intercept messages at specific stages around a `MessageChannel`'s send and receive operations by way of the `org.springframework.messaging.support.ChannelInterceptor` interface.
...
Spring Integration provides `MessagingTemplate` as a very easy way to integrate a messaging system into applications. It supports common message channel operations, such as send and receive. `MessagingTemplate` can support transactions by accepting a `PlatformTransactionManager`. Additionally, The `MessagingTemplate` can build Spring Integration `Message` instances.
