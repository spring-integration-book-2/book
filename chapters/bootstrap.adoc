...
To appreciate integration, we'll need to learn about Spring. What is Spring? What is dependency injection? Why do we use Spring Boot over (or in addition to?) Spring Framework? This chapter is a primer on the basics. We don't talk about integration in this chapter at all. If you already know the fundamentals of dependency injection, inversion-of-control, Java configuration, Spring application contexts, aspect-oriented programming (AOP), and Spring Boot auto-configuration, skip this chapter!
...
`Web` brings in everything you'd need today to build an application based on the Servlet specification and traditional Spring MVC. It brings in form validation, JSON and XML marshaling, WebSocket support, REST- and HTTP-controller support, etc. 

`JDBC` brings in support, like the `JdbcTemplate`, for working with SQL-based databases. `H2` is an embedded SQL database that will lose its state on every restart. This ease of use is ideal for our first steps since we won't install a database (or reset it).

`Lombok` is a compile-time annotation processor generating things like getters, setters, `toString()` methods, `equals()` methods, and so much more with but a few annotations. 

`Actuator` contributes HTTP endpoints, under `/actuator/\*`, giving visibility into the application's state.

There are some other choices if we're so inclined. What version of the JVM do you want to target? (I'd recommend using the version corresponding to the latest OpenJDK build.) What language do you want? We'll explore Kotlin later, but let's use Java for now. 

For all that we _did_ specify, we _didn't_ specify Spring itself. Or a logging library. Or any of a dozen framework-y bits that we'll need to do the job. Those are implied in the other dependencies when using Spring Boot, so we don't need to worry about them.

We'll get to integration, I promise, but nothing we're introducing in this section concerns itself with integration simply because we need to have a baseline. These days, it's fair game to assume you've some familiarity with JDBC (the Java Database Connectivity API) and the Servlet specification (there's traditionally been very little done on the web tier in Java that doesn't involve the Servlet specification).

Scroll to the bottom and select `Generate Project`.

.This shows us the selections on the Spring Initializr for a new application.
image::{code}/bootstrap/the-spring-initializr.png[alt=the Spring Initializr]
...
----
include::{code}/bootstrap/includes/demo-tree.txt[the tree of the Maven project generated from the Spring Initializr]
----
...
Our Maven build file, `pom.xml`, is rather plain. Each checkbox selected on the Spring Initializr is represented as a dependency in the `pom.xml` build file. Most of them will have the `groupId` `org.springframework.boot`. We selected `Web`, and that corresponds to a dependency for building web applications with `artifactId` `spring-boot-starter-web`, for example. That explains where at least three of our dependencies came from. Let's take a closer look. Testing is important, and we're in the future, and in the future, everybody tests.  You'll see at least `spring-boot-starter-test` among the dependencies added to your Maven build. The Spring Initializr often adds other testing libraries where appropriate based on the libraries you've added. There's no "opt-in" for it. The Spring Initializr generates new projects with test dependencies automatically.
...
This basic Spring Boot application comes with a `public static void main(String[] args)` entry-point class, `BootstrapApplication.java`. It is an empty class with a `main` method and an annotation, and it is _glorious_! While I'd love to stay here in the land of Lilliputian to jump right into bombastic Spring Boot, this wouldn't be much of a _bootstrap_ lesson without some background! So, delete `BootstrapApplication.java`. We'll get there. But first, some basics.

== Bean by the bag

Spring Framework, the first project to bear the Spring moniker, is at its core a dependency injection framework. Dependency injection frameworks are simple things with very profound possibilities. The idea is simple: applications change a lot. Decoupling helps to reduce the cost of changing our application code and system architecture. So we write code that separates behavior from implementation. Dependencies may originate from anywhere, but we can control where that happens.
...
Now, when you run `mvn clean package`, you'll get a working `.jar` in the `target` directory. You can do any of some things with it from there. You might deploy it to a cloud platform, like Cloud Foundry, like this:
...
You could also easily containerize the application with Spring Boot's built-in support for OCI/Docker images. Ensure you have the Docker daemon running somewhere, then run `mvn clean package spring-boot:build-image` and stand back. It might take a minute, but when the process completes you'll be rewarded with a Docker image that you can then `docker tag` and `docker push` to any container registry of your choice. (Might I recommend https://docs.pivotal.io/vmware-harbor/index.html[VMWare's Harbor])

You could then deploy that container itself to Cloud Foundry, or to a Kubernetes distribution like https://docs.vmware.com/en/VMware-Tanzu-Kubernetes-Grid/index.html[Tanzu Kubernetes Grid].



== Summary

In this chapter, we've examined the logical evolution of an application from basic to "bootiful." We've hopefully established that Spring Boot is a force multiplier. Hopefully, you're clear on how it could be used to build an application that's destined for production. Hopefully, you're clear on the basic workflow. You start at http://start.spring.io[the Spring Initializr], you add the relevant "starter" dependencies to activate desired functionality, and then configure the application, if necessary, by changing properties and contributing objects. From here, the world is your oyster! There are dozens of VMware-sponsored projects supporting Spring and far more besides maintained by third parties. Want to work with Microsoft Azure? Use their Spring Boot support. Want to work with Alibaba's Cloud? Use Spring Cloud Alibaba. Want to work with Google Cloud Platform? Check out Spring Cloud GCP. Want to work on Amazon Web Services? Check out Spring Cloud AWS! The list of interesting third party APIs goes on and on.

The world is growing increasingly distributed, and the systems and services requiring integration more numerous. We've looked at what it takes to build a Spring Boot-based service and we have a sense of some of the patterns required to get there. Spring Boot will serve as the foundation for our messaging- and Spring Integration-centric services.

Spring Integration takes the concepts introduced in this chapter - inversion of control, patterns, decoupling - and extends them in a natural way to the realm of messaging and integration. We'll see that Spring Integration decouples our business logic from the _where_ and _how_ of service integrations in the same way that Spring does for component dependencies.
